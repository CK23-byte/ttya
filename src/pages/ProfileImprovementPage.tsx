...
interface AvatarConfig {
  ...
  isProcessing?: boolean // True if avatar is still being generated by Simli (can take hours)
}
...
export default function ProfileImprovementPage() {
  ...
  const handleCreateAvatar = async () => {
    ...
    try {
      ...
      // Handle 202 Accepted (processing asynchronously)
      if (response.status === 202 || data.processing) {
        setAvatarCreationProgress(100)
        setAvatarCreationStatus('completed')

        // Store the request ID for potential status polling
        const requestId = data.requestId || data.faceId
        if (requestId) {
          setProfileData(prev => ({
            ...prev,
            avatarConfig: {
              type: 'custom',
              customAvatarId: requestId,
              customAvatarName: faceName,
        ...
        showModal(
          'Avatar Submitted for Processing',
          `Your avatar "${faceName}" has been submitted for creation.\n\n⚠️ Important: Avatar generation can take several hours (up to 2-3 hours).\n\nYou will be notified when your avatar is ready. In the meantime, you can continue using the app.`,
          'info'
        )
        return
      }
      ...
      if (!response.ok) {
        throw new Error(data.details || data.error || 'Failed to create face')
      }

      // Immediate success (face ID returned)
      if (data.faceId) {
        setAvatarCreationProgress(100)
        setProfileData(prev => ({
          ...prev,
        ...
        // No face ID in response - show the full response for debugging
        logger.error('Unexpected response from Simli:', data)
        throw new Error(data.hint || 'No face ID returned from Simli. Check server logs for details.')
      }
                      ...
                      {isCreatingAvatar && (
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-center text-pink-700">
                            {avatarCreationStatus === 'uploading' && 'Uploading and preparing photo...'}
                            {avatarCreationStatus === 'processing' && 'Submitting to Simli AI for avatar generation...'}
                          </p>
                          <p className="text-xs text-center text-gray-500 mt-1">
                            ⏱️ Note: Avatar generation can take several hours
                          </p>
                        </div>
                      )}
                    </div>
